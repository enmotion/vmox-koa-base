...
;  /**
  ...
  public constructor(model: Model<T>) {
    this.model = model;
    this.immutableKeys = Object.keys(this.model.schema.paths).map(key=>key).filter((key)=>!!this.model.schema.path(key).options.immutable)
  }
  /**
   * 创建并保存文档到数据库
   ...
   * @throws 转换后的业务可读错误（通过mongoDBErrorTransform处理原生错误）
   */
  public async updateMany (filter: RootFilterQuery<T>, update: Record<string, any>, options?: (MongoDB.UpdateOptions & MongooseUpdateQueryOptions<T>) | null) {
    try {
   ...
   * @throws 转换后的业务可读错误（通过mongoDBErrorTransform处理原生错误）
   */
  public async updateOne (filter: RootFilterQuery<T>, update: Record<string, any>, options?: (MongoDB.UpdateOptions & MongooseUpdateQueryOptions<T>) | null) {
    try {
