...
import { ClientService } from "./service";  // 用户服务层
import type { IClient } from "./schema";  // 用户模型接口
...
/**
 * 用户控制器类
 * @template T 扩展自IClient的泛型类型
 */
export class ClientControllers<T extends IClient> {
  public service: ClientService<T>;  // 用户服务实例
   ...
   * @param schema 用户数据模式
   */
  public constructor(service: ClientService<T>, schema: Schema<T>) {
